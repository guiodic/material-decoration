...
QColor Button::backgroundColor() const
{
    ...
    //--- CloseButton and ExcludeFromCaptureButton
    if (type() == KDecoration3::DecorationButtonType::Close
#if HAVE_EXCLUDE_FROM_CAPTURE
        || type() == KDecoration3::DecorationButtonType::ExcludeFromCapture
#endif
    ) {
...
#if HAVE_EXCLUDE_FROM_CAPTURE
        if (type() == KDecoration3::DecorationButtonType::ExcludeFromCapture && isChecked() && !isHovered() && !isPressed()) {
            return hoveredColor;
        }
#endif
        ...
        if (type() == KDecoration3::DecorationButtonType::ExcludeFromCapture) {
            if (isChecked()) {
                if (isPressed()) {
                    ...
                    return KColorUtils::mix(hoveredColor, pressedColor, m_transitionValue);
                }
                return hoveredColor;
            }
            return normalColor;
    ...
    if (isChecked() && type() != KDecoration3::DecorationButtonType::Maximize) {
        ...
        if (deco->m_internalSettings->useSystemColors()) {
            normalColor = qApp->palette().color(decoratedClient->isActive() ? QPalette::Active : QPalette::Inactive, QPalette::Highlight);
            hoveredColor = normalColor;
            pressedColor = normalColor.lighter();
        } else {
            normalColor = deco->titleBarForegroundColor();
            hoveredColor = KColorUtils::mix(
                deco->titleBarBackgroundColor(),
                deco->titleBarForegroundColor(),
                0.8);
            pressedColor = KColorUtils::mix(
                deco->titleBarBackgroundColor(),
                deco->titleBarForegroundColor(),
                0.7);
        }
    ...
    QColor hoveredColor;
    if (deco->m_internalSettings->useSystemColors()) {
        hoveredColor = qApp->palette().color(decoratedClient->isActive() ? QPalette::Active : QPalette::Inactive, QPalette::Highlight);
    } else {
        hoveredColor = KColorUtils::mix(
            deco->titleBarBackgroundColor(),
            deco->titleBarForegroundColor(),
    ...
    if (isPressed()) {
        QColor pressedColor;
        if (deco->m_internalSettings->useSystemColors()) {
            pressedColor = hoveredColor.lighter();
        } else {
            pressedColor = KColorUtils::mix(
                deco->titleBarBackgroundColor(),
                deco->titleBarForegroundColor(),
...
QColor Button::foregroundColor() const
{
    ...
    if (isChecked() && type() != KDecoration3::DecorationButtonType::Maximize) {
        ...
        QColor activeColor;
        if (deco->m_internalSettings->useSystemColors()) {
            const QColor highlightColor = qApp->palette().color(decoratedClient->isActive() ? QPalette::Active : QPalette::Inactive, QPalette::Highlight);
            const qreal l1 = luma(highlightColor);
            const qreal l2 = luma(deco->titleBarForegroundColor());
            if (qAbs(l1 - l2) < 0.3) {
                activeColor = (l1 > 0.5) ? Qt::black : Qt::white;
            } else {
                activeColor = deco->titleBarForegroundColor();
            }
        } else {
            activeColor = KColorUtils::mix(
        ...
        if (isPressed() || isHovered()) {
            ...
            if (deco->m_internalSettings->useSystemColors()) {
                ...
                const qreal l2 = luma(deco->titleBarForegroundColor());
                if (qAbs(l1 - l2) < 0.3) {
                    targetColor = (l1 > 0.5) ? Qt::black : Qt::white;
                } else {
                    targetColor = deco->titleBarForegroundColor();
                }
            } else {
                targetColor = deco->titleBarOpaqueBackgroundColor();
            }
